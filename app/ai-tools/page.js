    ...
    <nav style={{ background: "rgba(5,7,15,0.97)", borderBottom: "1px solid rgba(249,115,22,0.18)", backdropFilter: "blur(16px)", position: "sticky", top: 0, zIndex: 100 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 1.5rem", height: "64px" }}>
        ...
        <div className="mobile-nav-links" style={{ display: "flex", gap: "2.5rem", alignItems: "center" }}>
          {NAV_LINKS.map((link) => (
            <Link key={link} href={NAV_PATHS[link] || "/"} style={{ fontFamily: "'Syne', sans-serif", fontSize: "0.88rem", fontWeight: 600, color: "rgba(255,255,255,0.6)", textDecoration: "none", letterSpacing: "0.02em", borderBottom: "2px solid transparent", paddingBottom: "2px" }}>{link}</Link>
          ))}
        </div>
...
export default function AIToolsPage() {
  ...
  const filtered = tools.filter(t => {
    ...
    const matchPricing = pricingFilter === "All" || (pricingFilter === "Free" && t.pricingType === "freemium") || (pricingFilter === "Paid" && t.pricingType === "paid");
    return matchCat && matchSearch && matchPricing;
  });

  return (
    // ✅ width 100% + overflowX hidden added
    <div style={{ minHeight: "100vh", background: "#05070f", color: "#fff", width: "100%", overflowX: "hidden" }}>
      ...
      <section style={{ position: "relative", overflow: "hidden", padding: "5rem 2rem 3.5rem", textAlign: "center" }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(2.2rem, 6vw, 4.5rem)", fontWeight: 800, lineHeight: 1.08, letterSpacing: "-0.04em", marginBottom: "1.1rem", color: "#fff" }}>
            Best Free AI Tools<br />
            <span style={{ background: "linear-gradient(90deg, #f97316, #ef4444)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>You Must Try in 2025</span>
          </h1>
      ...
      <div className="filter-row" style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 1.5rem 1.5rem", display: "flex", gap: "1rem", alignItems: "center" }}>
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input className="search-input" type="text" placeholder="Search tools..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: "100%", padding: "0.75rem 1rem 0.75rem 2.7rem", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", color: "#fff", fontFamily: "'Outfit', sans-serif", fontSize: "0.92rem", transition: "border-color 0.2s, box-shadow 0.2s" }} />
        </div>
        <div style={{ display: "flex", gap: "0.5rem" }}>
          {["All", "Free", "Paid"].map(p => (
            <button key={p} className="cpill" onClick={() => setPricingFilter(p)} style={{ padding: "0.6rem 1.1rem", borderRadius: "10px", background: pricingFilter === p ? (p === "Free" ? "#10b981" : p === "Paid" ? "#ef4444" : "#f97316") : "rgba(255,255,255,0.05)", color: pricingFilter === p ? "#fff" : "#475569", fontFamily: "'Syne', sans-serif", fontSize: "0.8rem", fontWeight: 700 }}>{p}</button>
          ))}
        </div>
      </div>
      ...
      {/* TOOLS GRID */}
      <main className="tools-grid" style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 1.5rem 3rem", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(310px, 1fr))", gap: "1.4rem" }}>
        {filtered.length === 0 ? (
          ...
          <div key={tool.name} className="tool-card" style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px", overflow: "hidden", display: "flex", flexDirection: "column" }}>
            <div style={{ background: `linear-gradient(135deg, ${tool.accent}18, rgba(5,7,15,0.8))`, padding: "1.5rem 1.5rem 1.2rem", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                  <div style={{ width: 48, height: 48, borderRadius: "12px", background: `${tool.accent}22`, border: `1px solid ${tool.accent}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.6rem" }}>{tool.emoji}</div>
                  <div>
                    <h3 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "1rem", color: "#fff", margin: 0 }}>{tool.name}</h3>
                    <span style={{ fontSize: "0.68rem", fontFamily: "'Syne', sans-serif", fontWeight: 700, color: tool.accent, textTransform: "uppercase", letterSpacing: "0.08em" }}>{tool.category}</span>
                  </div>
                </div>
                <span style={{ padding: "0.22rem 0.65rem", borderRadius: "8px", background: tool.pricingType === "freemium" ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.15)", color: tool.pricingType === "freemium" ? "#10b981" : "#ef4444", border: `1px solid ${tool.pricingType === "freemium" ? "#10b98140" : "#ef444440"}`, fontSize: "0.68rem", fontFamily: "'Syne', sans-serif", fontWeight: 700, letterSpacing: "0.05em" }}>{tool.pricingType === "freemium" ? "FREE" : "PAID"}</span>
              </div>
            </div>
            <div style={{ padding: "1.2rem 1.5rem", flex: 1, display: "flex", flexDirection: "column", gap: "1rem" }}>
              <p style={{ color: "#475569", fontSize: "0.85rem", lineHeight: 1.65, margin: 0, fontFamily: "'Outfit', sans-serif" }}>{tool.description}</p>
              <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                {tool.tags.map(tag => (
                  <span key={tag} style={{ padding: "0.18rem 0.6rem", borderRadius: "6px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", color: "#334155", fontSize: "0.7rem", fontFamily: "'Outfit', sans-serif" }}>#{tag}</span>
                ))}
              </div>
              <a href={tool.url} target="_blank" rel="noopener noreferrer" className="visit-btn" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", background: `linear-gradient(135deg, ${tool.accent}, ${tool.accent}bb)`, color: "#fff", padding: "0.65rem 1rem", borderRadius: "10px", textDecoration: "none", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "0.85rem", transition: "opacity 0.2s, transform 0.2s", marginTop: "auto", boxShadow: `0 4px 20px ${tool.accent}30` }}>
                Try {tool.name} Free →
              </a>
            </div>
          </div>
      ...
      {/* Stats Bar */}
      <div style={{ background: "rgba(249,115,22,0.06)", borderTop: "1px solid rgba(249,115,22,0.12)", borderBottom: "1px solid rgba(249,115,22,0.12)", padding: "2rem 1.5rem" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto", display: "flex", justifyContent: "center", gap: "4rem", flexWrap: "wrap" }}>
          {[{ num: `${tools.length}+`, label: "AI Tools" }, { num: `${tools.filter(t => t.pricingType === "freemium").length}`, label: "Free to Use" }, { num: `${toolCategories.length - 1}`, label: "Categories" }, { num: "2025", label: "Updated" }].map(stat => (
